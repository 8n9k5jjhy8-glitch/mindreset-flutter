...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _loadSessions, child: _buildBody(l10n)),
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if ((session.resultNote ?? '').trim().isNotEmpty) ...[
            const SizedBox(height: 10),
            Text(
              _resultText(context, session.resultNote!.trim()),
              style: const TextStyle(
                fontSize: 12,
                height: 1.4,
            ...
            const SizedBox(height: 10),
            Text(
              _commentText(context, session.note!.trim()),
              style: const TextStyle(
                fontSize: 12,
                height: 1.4,
                color: Color(0xFF7A8477),
...
String _localizedModeTitle(BuildContext context, String modeKey) {
  ...
  switch (modeKey.trim().toLowerCase()) {
    case 'need_energy':
    case 'energy':
      return l10n.modeNeedEnergy;
    case 'sleep':
    case 'sleep_preparation':
      return l10n.modeSleepPreparation;
    case 'calm':
    case 'calm_mode':
      return l10n.modeCalm;
    case 'recovery':
      return l10n.modeRecovery;
    case 'focus':
    case 'want_to_focus':
      return l10n.modeFocus;
    case 'quick_reset':
      return l10n.modeQuickReset;
    case 'urgent_help':
      return l10n.modeUrgentHelp;
    case 'soft_support':
      return l10n.modeSoftSupport;
    case 'suitable_help':
      return l10n.modeSuitableHelp;
    case 'visual_contact':
      return l10n.modeVisualContactTitle;
    case 'custom':
      return l10n.modeSuitableHelp;
    default:
      return _localizedModeTitleByKey(modeKey);
  }
}

String _localizedModeTitleByKey(String modeKey) {
  switch (modeKey.trim().toLowerCase()) {
    case 'need_energy':
    case 'energy':
      return 'energy';
...
String _localizedStressTitle(BuildContext context, {required String rawTitle}) {
  final l10n = AppLocalizations.of(context)!;
...
String _resultText(BuildContext context, String value) {
  final code = Localizations.localeOf(context).languageCode;
  switch (code) {
    case 'en':
      return 'Result: $value';
...
String _commentText(BuildContext context, String value) {
  final code = Localizations.localeOf(context).languageCode;
  switch (code) {
    case 'en':
