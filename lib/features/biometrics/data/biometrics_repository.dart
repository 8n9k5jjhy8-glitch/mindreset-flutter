...
class BiometricsRepository {
  ...
  Future<BiometricsSnapshot> fetchLatestSnapshot() async {
    try {
      ...
      final activityDeviationScore = _computeActivityDeviationScore(cleaned, now);
      final sleepQualityScore = _computeSleepQualityScore(cleanedSleep, now);
      const double? wristTemperatureDelta = null;
      const int historyScore = 0;
      ...
      final stressIndex = _computeStressIndex(
        ...
        activityDeviationScore: activityDeviationScore,
      ...
      final availableMetrics = <String>[
        ...
        if (sleepQualityScore != null) 'sleepQualityScore',
        if (activityDeviationScore != null) 'activityDeviationScore',
      ];
  ...
  int _computeStressIndex({
    ...
    final rhrScore = _computeRestingHeartRateScore(restingHeartRate);
    if (rhrScore != null) {
      weightedScores.add(rhrScore * 0.20);
      weights.add(0.20);
    }

    final currentHeartRateScore = _computeCurrentHeartRateScore(
      heartRate: heartRate,
      restingHeartRate: restingHeartRate,
    );
    if (currentHeartRateScore != null) {
      weightedScores.add(currentHeartRateScore * 0.10);
      weights.add(0.10);
    }
    ...
    final totalScore = weightedScores.fold<double>(0, (sum, item) => sum + item);
  ...
  int? _computeRestingHeartRateScore(int? restingHeartRate) {
    if (restingHeartRate == null) {
      return null;
    }

    if (restingHeartRate <= 55) return 10;
    if (restingHeartRate <= 62) return 18;
    if (restingHeartRate <= 68) return 28;
    if (restingHeartRate <= 74) return 42;
    if (restingHeartRate <= 82) return 58;
    if (restingHeartRate <= 90) return 74;
    if (restingHeartRate <= 100) return 86;
    return 95;
  }

  int? _computeCurrentHeartRateScore({
    required int? heartRate,
    required int? restingHeartRate,
  }) {
    if (heartRate == null) {
    ...
    if (restingHeartRate != null) {
      final delta = heartRate - restingHeartRate;

      if (delta <= 5) return 12;
      if (delta <= 12) return 24;
      if (delta <= 20) return 40;
      if (delta <= 30) return 58;
      if (delta <= 40) return 76;
      return 90;
    }

    if (heartRate <= 65) return 12;
    if (heartRate <= 75) return 22;
    if (heartRate <= 85) return 36;
    if (heartRate <= 95) return 54;
    if (heartRate <= 110) return 74;
    return 90;
  }
  ...
  int? _computeRespiratoryScore(double? respiratoryRate) {
    ...
    if (respiratoryRate <= 13) return 10;
    if (respiratoryRate <= 15) return 18;
    if (respiratoryRate <= 17) return 30;
    if (respiratoryRate <= 19) return 44;
    if (respiratoryRate <= 22) return 62;
    if (respiratoryRate <= 25) return 80;
    return 92;
  }
  ...
  int? _computeWristTemperatureScore(double? wristTemperatureDelta) {
    ...
    if (absDelta < 0.2) return 10;
    if (absDelta < 0.4) return 28;
    if (absDelta < 0.7) return 52;
    if (absDelta < 1.0) return 74;
    return 90;
  }
  ...
  int? _computeActivityDeviationScore(
    ...
    if (stepsToday < 1500) return 72;
    if (stepsToday < 3000) return 55;
    if (stepsToday < 5000) return 35;
    if (stepsToday < 8000) return 20;
    return 12;
  }
