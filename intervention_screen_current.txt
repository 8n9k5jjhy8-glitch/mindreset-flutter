...
class _InterventionScreenState extends State<InterventionScreen> {
  ...
  void initState() {
    ...
    if (!_isPreview &&
        (initialStatus == 'started' || initialStatus == 'in_progress')) {
      _sessionStarted = true;
    }
    if (!_isPreview && initialStatus == 'completed') {
      _sessionStarted = true;
      _sessionCompleted = true;
    }
    if (!_isPreview && initialStatus == 'cancelled') {
  ...
  Future<void> _saveTrustedContact() async {
    ...
    try {
      ...
      if (!_sessionCompleted && !_sessionCancelled && vm.sessionId.isNotEmpty) {
  ...
  Future<void> _startPreviewSession() async {
    if (_isStarting || !_isPreview) return;
  ...
  Future<void> _handleClose() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Сеанс не завершён'),
          behavior: SnackBarBehavior.floating,
        ),
      );
  ...
  String get _primaryActionDescription {
    if (_isPreview) {
      return 'На основе анализа состояния выбран формат: ${vm.title}. Нажмите «Начать сеанс», чтобы открыть его, или вернитесь на главный экран.';
    }
    if (_sessionCompleted) {
  ...
  Widget build(BuildContext context) {
    ...
    final step = vm.step;
    final displayTitle =
        _isPreview ? 'Подходит формат помощи' : vm.title;

    if (_isPreview && !vm.isTrustedContact) {
      return Scaffold(
        ...
        body: SafeArea(
          child: ListView(
            ...
            children: [
              _HeroCard(
                vm: vm,
                displayTitle: 'Подходит короткая сессия помощи',
                isPreviewForUi: true,
              ),
              const SizedBox(height: 16),
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    const Text(
                      'Начните с этого формата помощи.',
                      style: TextStyle(
                        color: AppColors.textPrimary,
                        fontSize: 24,
                        height: 1.2,
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _isStarting ? null : _startPreviewSession,
                        ...
                        child: Text(
                          _isStarting ? 'Запуск…' : 'Начать сеанс',
                          style: const TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 16,
                          ),
        ...
        child: ListView(
          ...
          children: [
            _HeroCard(
              ...
              isPreviewForUi: _isPreview,
            ),
            const SizedBox(height: 16),
            if (_isPreview) ...[
              _PreviewStartCard(onStart: _startPreviewSession),
              const SizedBox(height: 16),
            ] else ...[
            ...
            if (vm.isTrustedContact)
              ...
              _PrimaryActionCard(
                ...
                isDisabled:
                    _isPreview ? false : (_sessionStarted || _sessionCompleted),
                onPressed: _isPreview ? _startPreviewSession : _startSession,
              ),
            const SizedBox(height: 16),
            _ActionButtonsBlock(
...
class InterventionViewModel {
  ...
  factory InterventionViewModel.fromArguments(
    ...
    final rawModeKey =
        (args['modeKey'] ?? args['sessionType'] ?? args['type'] ?? '')
            .toString()
            .trim();
    ...
    final title = _resolveTitle(rawTitle, sessionType);
    final step = _resolveStep(sessionType, args);
  ...
  static Color _resolveAccentColor(String sessionType, InterventionStep step) {
    ...
    if (normalized.contains('trustedcontact')) return const Color(0xFFA17456);
    if (normalized.contains('visualcontact')) return const Color(0xFF7B69A7);
  ...
  static IconData _resolveIcon(String sessionType, InterventionStep step) {
    ...
    if (normalized.contains('trustedcontact')) {
      return Icons.phone_in_talk_rounded;
    }
    if (normalized.contains('visualcontact')) return Icons.videocam_rounded;
...
class _HeroCard extends StatelessWidget {
  const _HeroCard({
    ...
    this.isPreviewForUi = false,
  });

  final InterventionViewModel vm;
  final String displayTitle;
  final bool isPreviewForUi;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  isPreviewForUi ? 'Рекомендация' : vm.step.label,
                  style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.w700,
                    fontSize: 12,
          ...
          const SizedBox(height: 10),
          const Text(
            'Формат подобран по вашему текущему состоянию.',
            style: TextStyle(
              color: Colors.white,
              fontSize: 16,
              height: 1.4,
...
class _PreviewStartCard extends StatelessWidget {
  const _PreviewStartCard({required this.onStart});
    ...
    if (isPreview) {
      return SizedBox(
        ...
        child: TextButton(
          ...
          child: Text(
            isClosing ? 'Закрытие...' : 'Отменить и вернуться',
            style: const TextStyle(
              color: AppColors.textSecondary,
              fontWeight: FontWeight.w700,
              fontSize: 15,
    ...
    return Column(
      children: [
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed:
                isCompleting || isClosing || !completeEnabled ? null : onComplete,
            style: OutlinedButton.styleFrom(
              minimumSize: const Size.fromHeight(52),
              side: BorderSide(
                color: AppColors.primary.withValues(alpha: 0.22),
